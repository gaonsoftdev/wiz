...
void CNewWizDialog::OnWizardBack()
{
	...
	if (lResult == -1)
	{
		DoneWizardBack(FALSE);
		return; // don't change pages
	}
	...
	if (lResult == 0)
	{
		...
		{
			DoneWizardBack(FALSE);
			return; // the current page was the first page
		}
...
void CNewWizDialog::OnWizardNext()
{
	...
	if (lResult == 0)
	{
		...
		if (Pos == NULL) {
			DoneWizardNext(FALSE);
			return; // the current page was the last page
		}
		pPage = (CNewWizPage*)m_PageList.GetAt(Pos);
	}
	else
...
void CNewWizDialog::DoneWizardBack(bool IsBack) {
	if (IsBack) {
...
void CNewWizDialog::DoneWizardNext(bool IsNext) {
	if (IsNext) {
		int activeIndex = GetActiveIndex();
	}
}
